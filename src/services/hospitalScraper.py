...
for tr in root.cssselect(".trust-list li a"):
    ...
    if (contacts_href.startswith('/Services/')):
        contactshtml = requests.get("http://www.nhs.uk/" + contacts_href).text
        ...
        postcode_txt = postcode_txt.strip().replace("\r\n","")

        # Get the list of hospitals
        hospAndClinics_html = requests.get("http://www.nhs.uk/" + hospAndClinics_href).text
        ...
        for hosp in hosplist_div:
            ...
            hosp_postcode_txt = hosp_address.split('\n')[-1] 
            if hosp_postcode_txt != '':
                #latlng = scraperwiki.geo.gb_postcode_to_latlng(hosp_postcode_txt)
                latlng = decodeAddressToCoordinates(hosp_postcode_txt)

                # latlng = [thelatlng[lat], thelatlng[lng]
                # latlng = [0,0]
            if latlng == None:
